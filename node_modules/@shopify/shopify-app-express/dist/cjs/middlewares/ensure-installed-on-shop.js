    ...
    return function ensureInstalledOnShop() {
        return async (req, res, next) => {
            ...
            if (!api.config.isEmbeddedApp) {
                ...
                return validateAuthenticatedSession.validateAuthenticatedSession({ api, config })()(req, res, next);
            }
            ...
            if (!session && !req.originalUrl.match(exitIframeRE)) {
                ...
                return redirectToAuth.redirectToAuth({ req, res, api, config });
            }
...
async function sessionHasValidAccessToken(api, config, session) {
    ...
    try {
        return (session.isActive(api.config.scopes) &&
            (await hasValidAccessToken.hasValidAccessToken(api, session)));
    }
    catch (error) {
