import { privacyTopics, BotActivityDetected, CookieNotFound, InvalidOAuthError } from '@shopify/shopify-api';
import { redirectToAuth } from '../redirect-to-auth.mjs';

async function authCallback({ req, res, api, config, }) {
    try {
    ...
    for (const topic in responsesByTopic) {
        ...
        for (const response of responsesByTopic[topic]) {
            if (!response.success && !privacyTopics.includes(topic)) {
                ...
                if (result.errors) {
                    config.logger.error(`Failed to register ${topic} webhook: ${result.errors[0].message}`, { shop: session.shop });
                }
                else {
                    config.logger.error(`Failed to register ${topic} webhook: ${JSON.stringify(result.data)}`, { shop: session.shop });
                }
            }
        }
    }
...
export { authCallback };
//# sourceMappingURL=auth-callback.mjs.map
